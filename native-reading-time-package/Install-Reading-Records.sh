...
if [ -d "$APP" ]; then
    backup="$BASE/diagnostics/ReadingRecords-v29.previous.$(date +%s)"
    mv "$APP" "$backup" 2>/dev/null || fail "无法备份旧 v29 UI"
fi
