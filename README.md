...
多台 Kindle 连接同一 Wi-Fi、保持唤醒并同时打开“阅读记录”后会自动同步。重复同步会按事件去重，不会重复累计时长；失败也不会影响本机记录。详见 [局域网同步说明](docs/lan-sync.md)。

## 版本

当前版本：`v1.3.6`（当日日期自动刷新、书库入口封面与局域网同步）
